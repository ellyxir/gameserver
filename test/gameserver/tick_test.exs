defmodule Gameserver.TickTest do
  ...
  describe "new/1" do
    test "generates a UUID id" do
      tick = Tick.new(transform: &{&1, :continue}, repeat_ms: 3000)
    ...
    test "stores the transform function" do
      ...
      tick = Tick.new(transform: transform, repeat_ms: 3000)
      assert tick.transform == transform
    end

    test "stores repeat_ms" do
      tick = Tick.new(transform: &{&1, :continue}, repeat_ms: 5000)
    ...
    test "accepts optional on_kill" do
      ...
      tick = Tick.new(transform: &{&1, :continue}, repeat_ms: 3000, on_kill: on_kill)
      assert tick.on_kill == on_kill
    end

    test "accepts optional kill_after_ms" do
      tick = Tick.new(transform: &{&1, :continue}, repeat_ms: 3000, kill_after_ms: 15_000)
